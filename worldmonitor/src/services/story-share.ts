...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via ${VARIANT_META[SITE_VARIANT]?.siteName ?? VARIANT_META.full.siteName} — situational intelligence (World Monitor engine, AGPL)`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief — ${VARIANT_META[SITE_VARIANT]?.siteName ?? VARIANT_META.full.siteName}`)}`,
